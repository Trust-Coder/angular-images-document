...
export class IvyPinch {
  ...
  handlePan = (event: any) => {
    if (
      this.scale < this.properties.minPanScale ||
      this.properties.disablePan
    ) {
      return;
    }
  ...
  handlePinch = (event: any) => {
    ...
    if (this.eventType === undefined || this.eventType === 'pinch') {
      ...
      this.moveX =
        this.initialMoveX -
        ((this.distance / this.initialDistance) * this.moveXC - this.moveXC);
      this.moveY =
        this.initialMoveY -
        ((this.distance / this.initialDistance) * this.moveYC - this.moveYC);
  ...
  limitPanY() {
    ...
    if (scaledImgHeight < parentHeight) {
      ...
      if (this.moveY > imgOffsetTop) {
        ...
        scaledImgHeight + Math.abs(imgOffsetTop) - parentHeight + this.moveY <
        0
      ) {
  ...
  limitPanX() {
    ...
    if (scaledImgWidth < parentWidth) {
      ...
      if (this.moveX > imgOffsetLeft) {
        ...
        scaledImgWidth + Math.abs(imgOffsetLeft) - parentWidth + this.moveX <
        0
      ) {
        this.moveX = -(
          imgWidth * this.scale +
  ...
  isDragging() {
    ...
    if (this.scale === 1) {
      return (
        imgHeight > this.parentElement.offsetHeight ||
  ...
  public setMoveX(value: number, transitionDuration: number): void {
    ...
    this.transformElement(
      transitionDuration || this.properties.transitionDuration
    );
  }
  ...
  public toggleZoom(event: any = false) {
    if (this.initialScale === 1) {
      if (event && event.changedTouches) {
        ...
        this.moveX =
          this.initialMoveX - (this.element.offsetWidth * (this.scale - 1)) / 2;
        this.moveY =
          this.initialMoveY -
  ...
  public setZoom(properties: { scale: number; center?: number[] }) {
    ...
    let scalingPercent =
      (visibleAreaWidth * this.scale) / (visibleAreaWidth * this.initialScale);
  ...
  public setTransform(properties: {
    ...
    const transitionDuration =
      properties.transitionDuration || this.properties.transitionDuration;
